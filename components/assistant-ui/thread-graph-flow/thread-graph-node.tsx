      ...
      <div
        ...
        style={{
          boxShadow: isSelected
            ? `0 24px 60px -30px ${accent}66, 0 0 0 1px ${accent}33`
            : undefined,
        } as CSSProperties}
      >
          ...
          <div className="relative flex items-start justify-between gap-3 pr-10">
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {typeof data.linkedArtifactCount === "number" &&
            ...
            <div className="flex flex-col items-end gap-1 text-[10px] uppercase tracking-[0.18em] text-slate-300/90">
              ...
              {typeof data.idx === "number" ? (
                <span>#{String(data.idx).padStart(2, "0")}</span>
              ) : null}
              {isSelected ? (
          ...
          {!isBridge && data.onContextScopeChange ? (
            <label className="nodrag nopan relative mt-3 block text-[11px] text-slate-300">
              ...
              >
                ...
                <option value="parent" disabled={isRoot}>
                  Parent message
                </option>
          ...
          {isRoot && (data.onToggleLinkEdit || data.onCopyGraphJson) ? (
            <div className="nodrag nopan relative mt-3 flex flex-wrap gap-2">
              {data.onToggleLinkEdit ? (
                <button
                  type="button"
                  className="rounded-full border border-white/15 bg-white/[0.05] px-3 py-1.5 text-xs text-slate-100 hover:bg-white/[0.1]"
                  onClick={(event) => {
                    event.stopPropagation();
                    data.onToggleLinkEdit?.();
                  }}
