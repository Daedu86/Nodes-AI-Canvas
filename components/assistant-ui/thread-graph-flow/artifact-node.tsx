  ...
  ({ id, data, selected, dragging }: NodeProps<ThreadGraphFlowNode>) => {
    ...
    const handleDelete = () => {
      const confirmed = window.confirm(
        `Delete ${data.title ?? "this artifact"}? The artifact and all of its canvas connections will be removed.`,
      );
      if (!confirmed) return;
      deleteArtifact(id);
          ...
          <div className="space-y-3">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="nodrag nopan flex shrink-0 items-center gap-1.5">
                ...
                <button
                  type="button"
                  className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-rose-500/35 bg-rose-500/10 text-rose-700 transition hover:bg-rose-500/20 dark:text-rose-300"
                  aria-label="Delete artifact node"
                  title="Delete this artifact and all of its canvas connections."
                  onClick={(event) => {
