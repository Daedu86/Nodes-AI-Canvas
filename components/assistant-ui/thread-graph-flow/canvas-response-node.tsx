...
export const CanvasResponseNode = memo(
  ({ data, selected }: NodeProps<ThreadGraphFlowNode>) => {
    const isAgentResponse = data.provider === "codex";
    return (
      <div
        className={[
      ...
      >
        <div className="relative overflow-hidden rounded-[15px] border border-white/8 bg-slate-950/95 px-4 py-3">
          ...
          <Handle type="target" position={Position.Left} className="!h-3 !w-3 !border-2 !border-slate-950 !bg-blue-300/90" style={{ left: -7 }} />
          <Handle type="source" position={Position.Right} className="!h-3 !w-3 !border-2 !border-slate-950 !bg-blue-300/90" style={{ right: -7 }} />
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className="inline-flex items-center gap-1 rounded-full border border-blue-300/35 bg-blue-400/12 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-blue-100">
                  <Bot className="h-3 w-3" /> {isAgentResponse ? "Agent response" : "Assistant"}
                </span>
                {data.modelLabel ? (
              ...
              <span className="rounded-full border border-white/10 bg-white/[0.04] px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-300/90">
                {isAgentResponse ? "Final answer" : "Result"}
              </span>
            </div>

            <div className="nowheel max-h-72 overflow-y-auto whitespace-pre-wrap break-words rounded-xl border border-white/10 bg-white/[0.05] px-3 py-2 text-sm leading-5 text-slate-100">
              {data.preview}
            </div>
          </div>
        </div>
