...
export const fileMemoryRepository: MemoryRepository = {
  ...
  async createMemoryItem(input: MemoryCreateInput) {
  ...
  async patchMemoryItem(memoryId, patch: MemoryPatch, ownerId) {
  ...
  async deleteMemoryItems(memoryIds, ownerId) {
    ...
    await Promise.all(
      uniqueIds.map((memoryId) => fileMemoryRepository.deleteMemoryItem(memoryId, ownerId)),
    );
  },
};
